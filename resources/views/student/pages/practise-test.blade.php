...
<div class="mdc-card mb-4">
    <h4 class="card-title">Practise Test</h4>
                                ...
                                <div class="card-body py-2">
                                   <div class="row">
                                       ...
                                       <div class="col-md-12 col-lg-4 col-xl-4 p-2">
                                            @if ($item['show'] == 0)
                                            <form action="{{ url('student/practisepaper') }}" method="post">
                                                @csrf
                                            ...
                                            @else
                                            <form action="{{ url('student/practise-result') }}" method="post">
                                                @csrf
                                                <input type="hidden" name="user" value="{{ Auth::guard('student')->User()->roll_no }}">
                                                <input type="hidden" name="subject" value="{{ $item['subject'] }}">
                                                <input type="hidden" name="class" value="{{ Auth::guard('student')->User()->class }}">
                                                <button type="submit" class="btn btn-sm btn-success">view Result</button>
                                            </form>
                                            @endif
                                       </div>
                                   </div>
